# cbonsai-saver

## A macOS screensaver based on [cbonsai](https://gitlab.com/jallbrit/cbonsai#cbonsai)

I really like cbonsai, but it's not a screensaver in the more modern sense. So here it is.

## Install

Install from the custom Homebrew tap:

```sh
brew tap le0-VV/cbonsai-saver https://github.com/le0-VV/cbonsai-saver
brew install cbonsai-saver
```

The Homebrew cask is Apple Silicon only. Intel Mac users should download the
`cbonsai-saver-<version>-x86_64.zip` archive from the GitHub release page and
install `cbonsai saver.saver` manually into `~/Library/Screen Savers`.

See [HOMEBREW.md](HOMEBREW.md) for tap and release maintenance notes.

## License

`cbonsai-saver` is licensed under the GNU General Public License version 3 or
later. Release builds bundle the original `cbonsai` executable; see
[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) for bundled component notices.
